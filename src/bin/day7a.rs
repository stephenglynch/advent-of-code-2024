...
async fn main(_spawner: Spawner) {
    info!("sizeof(Operands) {}", size_of::<Operands>());
    info!("sizeof(Equation) {}", size_of::<Equation>());
    info!("sizeof(EquationList) {}", size_of::<EquationList>());
    info!("sizeof(Operators) {}", size_of::<Operators>());
    
    let start = Instant::now();
    let (_, equations) = parse(INPUT_CONTENT).unwrap();
    info!("found {} equations", equations.len());
